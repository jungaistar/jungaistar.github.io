...
# 페이지 렌더링 검사는 al-folio 데모 글(blog/2021/distill)이 있어야 동작한다.
# 개인 사이트에서 데모 글을 지우면 대상 페이지가 없어 코드 문제가 없는데도
# 실패하므로 건너뛴다. layout: distill 글을 쓰면 자동으로 다시 검증된다.
# 아래 distill 런타임 해시 검사는 콘텐츠와 무관하므로 항상 수행한다.
if [ -f "${distill_page}" ]; then
  ...
  echo "skipping distill page checks: ${distill_page#"${tmp_site}"} not generated by this site"
fi
